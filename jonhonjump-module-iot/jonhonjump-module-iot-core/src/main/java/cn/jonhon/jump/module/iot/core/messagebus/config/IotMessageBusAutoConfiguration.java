package cn.jonhon.jump.module.iot.core.messagebus.config;

import cn.jonhon.jump.framework.mq.redis.core.RedisMQTemplate;
import cn.jonhon.jump.framework.mq.redis.core.job.RedisPendingMessageResendJob;
import cn.jonhon.jump.framework.mq.redis.core.job.RedisStreamMessageCleanupJob;
import cn.jonhon.jump.framework.mq.redis.core.stream.AbstractRedisStreamMessage;
import cn.jonhon.jump.framework.mq.redis.core.stream.AbstractRedisStreamMessageListener;
import cn.jonhon.jump.module.iot.core.messagebus.core.IotMessageBus;
import cn.jonhon.jump.module.iot.core.messagebus.core.local.IotLocalMessageBus;
import cn.jonhon.jump.module.iot.core.messagebus.core.redis.IotRedisMessageBus;
import cn.jonhon.jump.module.iot.core.messagebus.core.rocketmq.IotRocketMQMessageBus;
...
public class IotMessageBusAutoConfiguration {
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "jonhonjump.iot.message-bus", name = "type", havingValue = "local", matchIfMissing = true)
    ...
    @Configuration
    @ConditionalOnProperty(prefix = "jonhonjump.iot.message-bus", name = "type", havingValue = "rocketmq")
    ...
    /**
     * 特殊：由于 JonhonjumpRedisMQConsumerAutoConfiguration 关于 Redis stream 的消费是动态注册，所以这里只能拷贝相关的逻辑！！！
     *
     * @see cn.jonhon.jump.framework.mq.redis.config.JonhonjumpRedisMQConsumerAutoConfiguration
     */
    @Configuration
    @ConditionalOnProperty(prefix = "jonhonjump.iot.message-bus", name = "type", havingValue = "redis")
