package cn.jonhon.jump.module.bpm.service.task;
...
import cn.jonhon.jump.framework.common.util.number.NumberUtils;
import cn.jonhon.jump.framework.common.util.object.ObjectUtils;
import cn.jonhon.jump.framework.common.util.object.PageUtils;
import cn.jonhon.jump.framework.datapermission.core.annotation.DataPermission;
import cn.jonhon.jump.framework.web.core.util.WebFrameworkUtils;
import cn.jonhon.jump.module.bpm.controller.admin.definition.vo.model.BpmModelMetaInfoVO;
import cn.jonhon.jump.module.bpm.controller.admin.task.vo.task.*;
import cn.jonhon.jump.module.bpm.convert.task.BpmTaskConvert;
...
import cn.jonhon.jump.module.bpm.enums.definition.*;
import cn.jonhon.jump.module.bpm.enums.task.BpmCommentTypeEnum;
import cn.jonhon.jump.module.bpm.enums.task.BpmReasonEnum;
import cn.jonhon.jump.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import cn.jonhon.jump.module.bpm.service.definition.BpmModelService;
import cn.jonhon.jump.module.bpm.service.definition.BpmProcessDefinitionService;
import cn.jonhon.jump.module.bpm.service.message.BpmMessageService;
import cn.jonhon.jump.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
import cn.jonhon.jump.module.system.api.dept.DeptApi;
...
import static cn.jonhon.jump.module.bpm.enums.ErrorCodeConstants.*;
import static cn.jonhon.jump.module.bpm.framework.flowable.core.enums.BpmnModelConstants.START_USER_NODE_ID;
import static cn.jonhon.jump.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
    ...
    @DataPermission(enable = false) // 关闭数据权限，避免查询不到用户数据。相关案例：https://gitee.com/zhijiantianya/jonhonjump-cloud/issues/ID1UYA
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 jonhonjump-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/jonhonjump-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/jonhonjump-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/jonhonjump-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
