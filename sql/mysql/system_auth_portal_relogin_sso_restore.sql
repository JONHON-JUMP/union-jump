...
SELECT 15, '系统安全', 2, '过期重登恢复子系统SSO（分钟）', 'system.auth.portal-relogin-sso-restore-minutes', '120', true,
       '主系统登录过期后，同账号在该分钟数内重新登录主系统，切换子系统时用新 token 静默换票（不弹第二次登录）；建议≥锁屏分钟数；填 0 表示关闭', '1', NOW(), '1', NOW(), b'0'
FROM DUAL
WHERE NOT EXISTS (
    SELECT 1 FROM infra_config WHERE config_key = 'system.auth.portal-relogin-sso-restore-minutes' AND deleted = b'0'
);
