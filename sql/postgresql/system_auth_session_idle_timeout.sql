...
SELECT 14, '系统安全', 2, '空闲锁屏时长（分钟）', 'system.auth.session-idle-timeout-minutes', '60', true,
       '用户无操作超过该分钟数后锁屏，需重新登录；填 0 表示关闭', '1', NOW(), '1', NOW(), 0
WHERE NOT EXISTS (
    SELECT 1 FROM infra_config WHERE config_key = 'system.auth.session-idle-timeout-minutes' AND deleted = 0
);
